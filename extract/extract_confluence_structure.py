...
def extract_structure_from_first_object(json_file_path, llm_fallback=False, model=None):
    ...
    """
    try:
        # Tentative directe de chargement du fichier entier
        with open(json_file_path, 'r', encoding='utf-8') as f:
            # Utiliser safe_json_load directement sur le fichier entier
            data = safe_json_load(f, log_prefix=json_file_path, llm_fallback=llm_fallback, model=model)
        ...
        # Si l'approche directe échoue, essayons avec une lecture partielle
        try:
            with open(json_file_path, 'r', encoding='utf-8') as file:
                ...
                try:
                    ...
                    # Si même la lecture partielle échoue, enregistrer l'erreur d'origine
                    return {
                        "filename": os.path.basename(json_file_path),
                        "error": f"Erreur de parsing JSON: {str(e)}. Détails: {str(e2)}",
                        "content_preview": content[:500] + "..."
                    }
        except Exception as e3:
            # Erreur lors de l'ouverture ou de la lecture du fichier
            return {
                "filename": os.path.basename(json_file_path),
                "error": f"Erreur lors de la lecture du fichier: {str(e3)}"
            }
